...
def _prune_stale_attempts_for_source_file(cursor, offering_id: int, current_source_file_id: int):
    cursor.execute(
        ...
        """,
        (offering_id, PTA_SOURCE_SYSTEM, current_source_file_id, current_source_file_id),
    )
...
def _recalc_student_assignment(cursor, offering_id: int, transcript_rows, student_no_to_id):
    ...
    cursor.execute(
        ...
        SELECT
          ...
          COALESCE(sps.best_total_score, 0) AS best_total_score
        ...
        LEFT JOIN (
          SELECT
            ...
            COALESCE(SUM(COALESCE(best_score, 0)), 0) AS best_total_score
    ...
    aggregates = {
        row[0]: {
            ...
            "first_submit_at": row[2],
            "last_submit_at": row[3],
            "accepted_problem_count": int(row[4] or 0),
            "best_total_score": float(row[5]) if row[5] is not None else None,
        }
    ...
    for student_id, problem_count in cursor.fetchall():
        ...
        latest_total_score = transcript_row["total_score"] if transcript_row else aggregate["best_total_score"]
        ranking = transcript_row["ranking"] if transcript_row else None
        if aggregate["submitted_problem_count"] == 0:
            submission_status = "NOT_STARTED"
        elif latest_total_score is not None:
...
def _sync_one_experiment(conn, crawl_dir: Path, exp_dir: Path, pta_user_map: dict, student_name_map: dict):
    ...
    try:
        ...
        _recalc_student_assignment(cursor, resolved["offering_id"], transcript_rows, student_no_to_id)
