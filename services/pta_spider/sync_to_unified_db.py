...
PTA_PROBLEM_DETAIL_FILE = "题目详情.json"
...
def _read_problem_detail_rows(json_path: Path):
    ...
    for item in data:
        if not isinstance(item, dict):
            continue
        rows.append({
            "problem_set_problem_id": str(item.get("problem_set_problem_id") or "").strip(),
            "problem_id": str(item.get("problem_id") or "").strip(),
            "problem_url": str(item.get("problem_url") or "").strip(),
            "title": str(item.get("title") or "").strip(),
            "score": item.get("score"),
            "difficulty": item.get("difficulty"),
            "difficulty_label": str(item.get("difficulty_label") or "").strip(),
            "knowledge_points": item.get("knowledge_points") or [],
        })
    return rows


def _build_problem_detail_map(problem_detail_rows):
    by_problem_set_problem_id = {}
    for row in problem_detail_rows:
        key = row.get("problem_set_problem_id")
        if key:
            by_problem_set_problem_id[key] = row
    return by_problem_set_problem_id


def _knowledge_summary(problem_detail):
    if not problem_detail:
        return {
            "pta_global_problem_id": None,
            "problem_url": None,
            "knowledge_path": None,
            "knowledge_leaf": None,
            "knowledge_point_ids": None,
            "knowledge_points_json": None,
            "pta_difficulty_level": None,
            "pta_difficulty_label": None,
        }
    knowledge_points = problem_detail.get("knowledge_points") or []
    paths = [kp.get("path") for kp in knowledge_points if kp.get("path")]
    leafs = [kp.get("leaf") for kp in knowledge_points if kp.get("leaf")]
    ids = [kp.get("ids") for kp in knowledge_points if kp.get("ids")]
    difficulty = problem_detail.get("difficulty")
    try:
        difficulty_level = int(difficulty) if difficulty is not None and str(difficulty).strip() != "" else None
    except (TypeError, ValueError):
        difficulty_level = None
    return {
        "pta_global_problem_id": problem_detail.get("problem_id") or None,
        "problem_url": problem_detail.get("problem_url") or None,
        "knowledge_path": "; ".join(paths) if paths else None,
        "knowledge_leaf": "; ".join(leafs) if leafs else None,
        "knowledge_point_ids": json.dumps(ids, ensure_ascii=False) if ids else None,
        "knowledge_points_json": json.dumps(knowledge_points, ensure_ascii=False) if knowledge_points else None,
        "pta_difficulty_level": difficulty_level,
        "pta_difficulty_label": problem_detail.get("difficulty_label") or None,
    }
...
def _ensure_assignment_problem(cursor, offering_id: int, pta_problem_id: str, cache: dict, title=None, sort_order=0, problem_detail=None):
    if not pta_problem_id:
        return None
    if pta_problem_id in cache:
        if problem_detail:
            _update_assignment_problem_knowledge(cursor, cache[pta_problem_id], problem_detail)
    ...
    if problem_detail:
        _update_assignment_problem_knowledge(cursor, problem_id, problem_detail)
    cache[pta_problem_id] = problem_id
    return problem_id


def _update_assignment_problem_knowledge(cursor, assignment_problem_id: int, problem_detail: dict):
    if not assignment_problem_id or not problem_detail:
        return
    column_values = _knowledge_summary(problem_detail)
    available = []
    for column_name, value in column_values.items():
        if _table_has_column(cursor, "assignment_problem", column_name):
            available.append((column_name, value))
    if not available:
        return
    set_clause = ", ".join(f"{column_name} = %s" for column_name, _ in available)
    values = [value for _, value in available]
    values.append(assignment_problem_id)
    cursor.execute(
        f"""
...
def _sync_one_experiment(conn, crawl_dir: Path, exp_dir: Path, pta_user_map: dict, student_name_map: dict):
    ...
    try:
        ...
        files = {}
        problem_detail_path = exp_dir / PTA_PROBLEM_DETAIL_FILE
        if problem_detail_path.exists():
            files["PROBLEM_DETAIL_JSON"] = (
                problem_detail_path,
                _register_source_file(cursor, import_job_id, problem_detail_path, crawl_dir, "PROBLEM_DETAIL_JSON"),
            )
        ...
        problem_detail_rows = _read_problem_detail_rows(files["PROBLEM_DETAIL_JSON"][0]) if "PROBLEM_DETAIL_JSON" in files else []
        problem_detail_by_id = _build_problem_detail_map(problem_detail_rows)
        ...
        for row in sorted(scored_code_rows, key=lambda item: (item["pta_problem_id"], item["student_no"], item["relative_name"])):
            ...
            problem_id = _ensure_assignment_problem(
                ...
                problem_detail=problem_detail_by_id.get(row["pta_problem_id"]),
            )
            student_id = student_no_to_id.get(row["student_no"])
            if not student_id:
