...
class PTAClient:
    ...
    def __init__(self, username=None, password=None, allow_env_fallback=True):
        ...
        env_password = os.getenv("PTA_PASSPORT") if allow_env_fallback else None
    ...
    def get_sets_requiring_content(self, all_sets):
        """Return new sets plus history entries whose local problem files were lost."""
        ...
        for ps in all_sets:
            ...
            if ps_id in pending_ids or not self.history.is_crawled(ps_id):
                continue
            problem_dir = self.crawl_dir / ps_name
            content_file = problem_dir / "题目内容.txt"
            detail_file = problem_dir / "题目详情.json"
            if not content_file.exists() or not detail_file.exists():
                print(f"本地题目内容/详情缺失，将重新抓取: {ps_name}")
                pending.append(ps)
    ...
    def _filter_problem_sets_by_keyword(self, problem_sets, keyword):
        normalized_keyword = self._normalize_problem_set_keyword(keyword)
        ...
        for problem_set in problem_sets:
            normalized_name = self._normalize_problem_set_keyword(problem_set.get("name", ""))
            if not normalized_name:
                continue
            if normalized_name == normalized_keyword or normalized_name.startswith(normalized_keyword):
                exact_or_prefix.append(problem_set)
                continue
            if normalized_keyword in normalized_name:
                contains.append(problem_set)

        filtered_sets = exact_or_prefix if exact_or_prefix else contains
        for problem_set in filtered_sets:
            print(f"  Keep: {problem_set.get('name', '?')}")
        return filtered_sets

    @staticmethod
    def _normalize_problem_set_keyword(value):
        if value is None:
            return ""
    ...
    def get_global_problem_detail(self, problem_id):
        """Get canonical PTA problem detail, including knowledge point paths."""
        return self.api_get(f"/problems/{problem_id}")
    ...
    @staticmethod
    def extract_knowledge_points(problem_detail):
        problem = problem_detail.get("problem", {}) if isinstance(problem_detail, dict) else {}
        paths = problem.get("knowledgePointPaths") or []
        result = []
        for path in paths:
            points = path.get("knowledgePoints") or []
            names = [str(p.get("name", "")).strip() for p in points if p.get("name")]
            ids = [str(p.get("id", "")).strip() for p in points if p.get("id")]
            if not names:
                continue
            result.append({
                "path": " / ".join(names),
        ...
        try:
            ...
            if problems:
                    ...
                    for p in problems:
                        ...
                        if pid:
                            try:
                                ...
                                global_problem_id = str(psp.get("problemId") or p.get("problemId") or "")
                                knowledge_points = []
                                difficulty = psp.get("difficulty", p.get("difficulty"))
                                difficulty_label = self.difficulty_label(difficulty)
                                if global_problem_id:
                                    try:
                                        global_detail = self.get_global_problem_detail(global_problem_id)
                                        global_problem = global_detail.get("problem", {}) if isinstance(global_detail, dict) else {}
                                        knowledge_points = self.extract_knowledge_points(global_detail)
                                        difficulty = global_problem.get("difficulty", difficulty)
                                        difficulty_label = self.difficulty_label(difficulty)
                                    except Exception as e:
                                        print(f"    知识点获取失败: {title} ({global_problem_id}): {e}")
                                if difficulty_label:
                                    f.write(f"难度: {difficulty_label}\n")
                                if knowledge_points:
                                    f.write("知识点: " + "; ".join(kp["path"] for kp in knowledge_points) + "\n\n")
                                f.write(f"{content}\n")
                                structured_problems.append({
                                    "problem_set_id": str(ps_id),
                                    "problem_set_problem_id": str(psp.get("id") or pid),
                                    "problem_id": global_problem_id,
                                    "problem_url": f"https://pintia.cn/problems/{global_problem_id}" if global_problem_id else "",
                                    "label": str(psp.get("label") or label or ""),
                                    "title": str(psp.get("title") or title or ""),
                                    "score": psp.get("score", p.get("score")),
                                    "type": psp.get("type", p.get("type")),
                                    "difficulty": difficulty,
                                    "difficulty_label": difficulty_label,
                                    "problem_pool_index": psp.get("problemPoolIndex", p.get("problemPoolIndex")),
                                    "index_in_problem_pool": psp.get("indexInProblemPool", p.get("indexInProblemPool")),
                                    "content": content,
                ...
                if structured_problems:
                    with open(base_dir / "题目详情.json", "w", encoding="utf-8") as jf:
                        json.dump(structured_problems, jf, ensure_ascii=False, indent=2)
        except Exception as e:
            print(f"  获取题目列表失败: {e}")
