...
- 让云服务器能够访问 `dl.google.com`、`googlechromelabs.github.io` 和 `storage.googleapis.com`，这样构建时可以安装 Chrome 和匹配驱动。
- 如果服务器网络访问 Google 源失败，优先配置服务器出网、代理或可用镜像源，再重新 `docker compose build`。
